    ...
    properties: {
      intent: {
        ...
        description:
          ...
          'IMPORTANT: classify what the LATEST message asks for. The earlier conversation is context only - if an exchange/refund/change was already handled and the latest message just acknowledges it, that is POSITIVE_FEEDBACK, not the original intent. EXCEPTION: if that acknowledgment also DESCRIBES a product defect or quality problem (the print was barely visible, a misprint, a smell, poor quality) - even gratefully and even though the replacement already shipped - classify it ORDER_ISSUE so the defect is surfaced, not POSITIVE_FEEDBACK. ' +
