...
7. DELIVERED BUT THE CUSTOMER CANNOT FIND IT: a package usually IS marked delivered, but carriers sometimes mark it a day or two early. Do NOT open with "good news, it was delivered" or tell them they already have it. HOW YOU ANSWER DEPENDS ON HOW OLD THE DELIVERY IS - check the Carrier Tracking facts for the delivery date and for a "STALE DELIVERY" line before you write.
   - RECENT delivery (under about two weeks ago, and no stale-delivery line in the facts). On their FIRST message about it: apologize, then LEAD with the facts we have - the delivery date/time/location and the proof-of-delivery or tracking link when the context provides them (the operator can attach a delivery photo). Keep the check-around advice to ONE short sentence (check the property and with household/neighbors, give it a day). If the delivery location looks like it might not match them, state the shipping address on the order and ask them to confirm it. Do NOT promise a replacement yet. Only if they write back having ALREADY looked and it is still missing: confirm we are sending a free replacement.
...
export class ClaudeService {
  ...
  private buildUserMessage(context: SuggestionContext): string {
    ...
    if (context.trackingInfo) {
      ...
      if (t.proofOfDeliveryUrl) {
        ...
        message += t.staleDelivery
          ...
          : `  When the customer says the package is lost or not received but the carrier shows DELIVERED, include this proof link in the reply and suggest checking with household members/neighbors and the exact drop spot shown.\n`;
      }
      message += '\n';
    }
    ...
    if (!context.refinement && context.fewShotExamples && context.fewShotExamples.length > 0) {
      ...
      message +=
        'THEY SHOW VOICE, NOT FACTS. Several of them state an action as ALREADY DONE ("I have already processed a full refund", "I have canceled your order"). Only ever write that when THIS thread\'s facts prove it - a refunded amount in the Order Context, a Recent Agent Action saying so, or an existing replacement order. Otherwise say what you are doing or offering, never that it is done. A draft that invents a completed refund is the worst error we make: the customer waits for money that is not coming.\n\n';
